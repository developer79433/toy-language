...
{
    map_buf_entry_list *entry_list = (map_buf_entry_list *) buf_list_alloc_2((toy_str *) &key, sizeof(key), buf, buf_size);
    assert(entry_list->entry.key == key);
    assert(0 == memcmp(&entry_list->entry.payload.c, buf, buf_size));
...
void map_buf_entry_list_payload_set_buf(map_buf_entry_list *list, void *buf, size_t buf_size)
{
    ...
    memcpy(&entry->payload.c, buf, buf_size);
}
