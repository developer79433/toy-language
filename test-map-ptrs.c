...
static item_callback_result map_item_callback(test_visitor *test_vis, const map_str_ptr_entry *entry)
{
...
void assert_one_bucket_not_null(map_str_ptr_entry_list **arr, size_t len)
{
    assert_ptr_array_one_not_null((void **) arr, len);
}

void test_map_str_ptr_basics(void)
{
    // Test create
    map_str_ptr *map1 = map_str_ptr_alloc();
    assert(0 == map_str_ptr_size(map1));
    ...
    // Test reset
    map_str_ptr_reset(map1);
    assert(0 == map_str_ptr_size(map1));

    // Test free
    map_str_ptr_free(map1);
}
...
static void insert_test_data(map_str_ptr *map)
{
    for (test_tuple *tuple = &test_data[0]; tuple < &test_data[ELEMENTSOF(test_data)]; tuple++) {
        set_result set_res = map_str_ptr_set(map, tuple->key, &tuple->value);
        assert(SET_NEW == set_res);
    }
}

static item_callback_result print_entry(map_visitor *visitor, map_str_ptr_entry *entry)
{
...
static void test_visitors(void)
{
    ...
    assert(ELEMENTSOF(test_data) == map_str_ptr_size(map1));
...
static toy_bool compare_value(void *cookie, map_str_ptr_entry *entry)
{
