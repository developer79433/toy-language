...
static item_callback_result map_item_callback(void *cookie, toy_str key, void *value)
{
    if (!test_map_entry(cookie, key, (toy_val *) value)) {
...
void test_map_ptrs(void)
{
    ...
    map_ptr_set(map1, "first key", &val1);
    ...
    map_ptr_set(map1, "second key", &val2);
    ...
    map_ptr_set(map1, "second key", &new_value);
    assert(1 == map_ptr_size(map1));
    toy_val *get6 = map_ptr_get(map1, "second key");
    assert(get6 == &new_value);
    ...
    enumeration_result res = map_ptr_foreach(map1, map_item_callback, &args);
