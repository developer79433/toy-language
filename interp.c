...
void dump_expr(const toy_expr *expr) {
    fprintf(stderr, "expr type %s", toy_expr_type_name(expr->type));
}

static void invalid_operand(enum toy_expr_type expr_type, const toy_expr *operand)
{
    dump_expr(operand);
    fatal_error("Invalid operand for expression %s", toy_expr_type_name(expr_type));
}

static void invalid_expr_type(enum toy_expr_type expr_type) {
    fatal_error("Invalid expression type %s", toy_expr_type_name(expr_type));
}

static void invalid_stmt_type(enum toy_stmt_type stmt_type)
{
    fatal_error("Invalid statement type %s", toy_stmt_type_name(stmt_type));
}

static void invalid_cast(enum toy_expr_type expr_type, const toy_expr *expr)
{
    dump_expr(expr);
