...
{
    assert(offsetof(map_buf_buf, buckets) == offsetof(generic_map, buckets));
    assert(offsetof(map_buf_buf, num_items) == offsetof(generic_map, num_items));
...
map_buf_buf *map_buf_buf_alloc(void)
{
    assert(offsetof(map_buf_buf, buckets) == offsetof(generic_map, buckets));
    assert(offsetof(map_buf_buf, num_items) == offsetof(generic_map, num_items));
    return (map_buf_buf *) map_alloc();
}

static map_buf_buf_entry_list **map_buf_get_bucket(map_buf_buf *map, const void *key, size_t key_len)
{
    assert(offsetof(map_buf_buf, buckets) == offsetof(generic_map, buckets));
    assert(offsetof(map_buf_buf, num_items) == offsetof(generic_map, num_items));
    return (map_buf_buf_entry_list **) map_get_bucket_ptr((generic_map *) map, key, key_len);
}

map_buf_buf_entry *map_buf_buf_get_entry(map_buf_buf *map, const void *key, size_t key_len)
{
    return (map_buf_buf_entry *) map_get_entry((generic_map *) map, key, key_len);
}

const map_buf_buf_entry *map_buf_buf_get_entry_const(const map_buf_buf *map, const void *key, size_t key_len)
{
    return (const map_buf_buf_entry *) map_get_entry_const((const generic_map *) map, key, key_len);
}

void *map_buf_buf_get(map_buf_buf *map, const void *key, size_t key_len)
{
    map_buf_buf_entry *entry = map_buf_buf_get_entry(map, key, key_len);
    if (entry) {
        return &entry->c;
    }
    return NULL;
...
{
    const map_buf_buf_entry *entry = map_buf_buf_get_entry_const(map, key, key_len);
    if (entry) {
        return &entry->c;
    }
    return NULL;
...
static item_callback_result map_buf_set_entry_callback(map_buf_set_visitor *set_vis, size_t index, map_buf_buf_entry_list *list)
{
    map_buf_buf_entry *entry = map_buf_entry_list_payload(list);
    if (
...
{
    map_buf_buf_entry_list *new_entry;
    map_buf_buf_entry_list **bucket = map_buf_get_bucket(map, key, key_len);
    if (*bucket) {
